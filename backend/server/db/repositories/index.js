class Repository {
  ...
  findRecord(where = {}, attributes = null, include = null, options = {}) {
    let params = { where };
    if (attributes) {
  ...
  findAndCountAllRecords(where = {}, attributes, include, options = {}, plain = false) {
    let params = {
      where: {
        ...where,
      },
    ...
    const instance = this.modelInstance.findAndCountAll(params);
    return plain ? instance.map((el) => el.get({ plain: true })) : instance;
  }
